...
def search(request):
    ...
    # Check if movie is present in local database
    try:
        movie = Movie.objects.get(title__icontains=query)
        movie_title = movie.title
        poster_url = movie.poster_url
    except Movie.DoesNotExist:
        # If movie is not present in local database, search for it using TMDB API
        api_key = '6e65f8bcc3b3ca0c03e49fbb21186861'
        ...
        movie_title = top_result.get('original_title')
        ...
        # Save the movie details to the local database
        try:
            movie = Movie.objects.create(title=movie_title, poster_url=poster_url)
        except Exception as e:
            return JsonResponse({'error': f'Error saving movie to database: {str(e)}'})
    
    # Return the movie details to the client
    return JsonResponse({'title': movie_title, 'poster_url': poster_url})
